...
arguments:
	...
	-custom
		If flag is present, script uses custom attack code even if the
		standard unit/attack.lua is available
...
argCustom = (string.match(dfhack.run_command_silent("unit/attack"),"selected")~="selected")

if args.custom or argCustom==true then
	attack_cmd = "tesb-attack"
else
	attack_cmd = "unit/attack"
end
...
dfhack.gui.makeAnnouncement(179,{false,false,false,true},xyz2pos(xyz[1] or -3000, xyz[2] or 0, xyz[3] or 0),text, color,1)
...
if args.location then
	...
	if math.random(0,2) == 2 then dfhack.run_command("modtools/create-unit -race TESB_PET_ROCK "..command) end
	if math.random(0,3) == 3 then dfhack.run_command("modtools/create-unit -race TESB_PET_ROCK "..command) end
end

if args.wyrm then
	dfhack.run_command("modtools/create-unit -race TESB_WYRM "..command.." -civId \\-1")
	...
	if args.miner then
		...
		dfhack.run_command(attack_cmd .. " -attacker " .. wyrm_id .. " -defender " .. args.miner .. " -attack SLAP")
	end
else
	if argFriendly == false then
		dfhack.run_command("modtools/create-unit -race TESB_AWAKENED_STONE "..command.." -civId \\-1")
		-- Induce attack if miner provided
		if args.miner then
			...
			dfhack.run_command(attack_cmd .. " -attacker " .. stone_id .. " -defender " .. args.miner .. " -attack SCRATCH")
		end
	else
		dfhack.run_command("modtools/create-unit -race TESB_AWAKENED_STONE "..command)
	end
end
