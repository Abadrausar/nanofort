...
preact.AddRecipe("DFHACK_POWERED_FL_AUTODECORATOR", {
	...
	output_item = function(items, wshop, extras)
		...
		if ok then
			...
			if extras.powder:isDye() == true then
				...
				if pitems.GetMRP("GLAZE_MAT", imp_mat) ~= nil and pfilter.RC("CAN_GLAZE")(item) then
					improvement.cover_flags.glazed = true
				end
			end
...
pwshops.Register("DFHACK_POWERED_FL_AUTODECORATOR", nil, 20, 0, 500, preact.MakeHandler("DFHACK_POWERED_FL_AUTODECORATOR", {
