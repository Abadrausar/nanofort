...
func main() {
	...
	flags.BoolVar(&ZapAddons, "zapaddons", ZapAddons, "Ignore any -addons flags loaded from rubble7.ini.")
	...
	flags.StringVar(&IARegion, "iapply", IARegion, "Name of a world (or 'raw' for the main raw directory) to apply a save safe addon to (use -addons to specify the addon).")
	...
	if TSetRegion != "" {
		err := rubble7.TSetModeRun(TSetRegion, RblDir, DFDir, *AddonsDir, *AddonsList, log)
		if Bench {
	...
	if IARegion != "" {
		err := rubble7.IAModeRun(IARegion, RblDir, DFDir, *AddonsDir, *AddonsList, log)
		if Bench {
			log.Println("Run time: ", time.Since(timeStart))
		}
		if err != nil {
