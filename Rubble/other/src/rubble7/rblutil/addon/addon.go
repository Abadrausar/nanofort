...
// Default tags are automatically applied based on file extension.
func NewFile(name, source string, content []byte) *File {
	...
	for _, tag := range GetFileTags(name) {
		file.Tags[tag] = true
	}
...
func (list *FileList) Update(data []*Addon) {
	if data == nil {
		return
	}
...
// If the addon slice is nil nothing is done.
func (list *FileList) UpdateFunc(data []*Addon, filter func(*File) bool) {
	if data == nil {
		return
	}
...
func (list *FileList) UpdateFilter(data []*Addon, filter map[string]bool) {
	if data == nil {
		return
	}
