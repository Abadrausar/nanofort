...
actions.RegisterFunc("merge", func(log *rblutil.Logger, rblDir, dfDir, outDir string, addonDirs []string, options []interface{}) bool {
	...
	rsource, err := axis.ReadAll(fs, ruleFile)
	if err != nil {
		log.Println(err)
		return false
	}
	...
	srcraws, err := axis.ReadAll(fs, sourceFile)
	if err != nil {
		log.Println(err)
		return false
	}
	...
	err = axis.WriteAll(fs, outFile, in)
	if err != nil {
		log.Println(err)
		return false
	}
