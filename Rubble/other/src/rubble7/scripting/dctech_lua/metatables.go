...
func rubbleStateAddonsTbl(l *lua.State) {
	...
	l.Push(func(l *lua.State) int {
		...
		i := int(l.ToInt(2))
		if i < 1 || i > len(state.Addons) {
			return 0
		}

		l.Push(state.Addons[i-1])
		addonTbl(l)
		l.SetMetaTable(-2)
		return 1
	})
...
func rubbleStateAddonsTblTbl(l *lua.State) {
	...
	l.Push(func(l *lua.State) int {
		...
		addon, ok := state.AddonsTbl[l.ToString(2)]
		if !ok {
			return 0
		}
...
func addonMetaTbl(l *lua.State) {
	...
	l.Push(func(l *lua.State) int {
		...
		case "Name":
			return 0
		case "Header":
			meta.Header = template.HTML(l.ToString(3))
			return 0
		case "Description":
			meta.Description = template.HTML(l.ToString(3))
			return 0
		case "DescFile":
			return 0
		case "Activates":
		...
		case "LoadPriority":
			return 0
		case "Author":
			meta.Author = l.ToString(3)
			return 0
		case "Version":
			meta.Version = l.ToString(3)
			return 0
		}
		return 0
	})
