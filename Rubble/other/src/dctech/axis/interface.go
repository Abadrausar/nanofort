...
// 
// AXIS is based on a simple interface that provides generic IO facilities. Specific implementations
// of this interface are used together to provide an easy to use OS-agnostic way to handle IO.
// 
// By default OS file and directory interfaces are provided in addition to logical "files" and
// "directories". Generally these are then bound together with a "FileSystem" and possibly a
// "Multiplexer".
// 
...
// DataSource is the heart of AXIS. EVERYTHING in AXIS implements DataSource, and it is the sole
// interface to all AXIS data.
type DataSource interface {
	...
	// Delete tries to delete an AXIS resource, this generally removes the backing file/directory
	// from the OS's file system.
	// 
	// Some resources cannot be deleted, in this case an Error with the type ErrReadOnly or
	// ErrBadAction is returned (whichever one make more sense).
	Delete(path *Path) error
	
	// Directories
	
	// Create tries to create an AXIS resource, this generally creates the backing file/directory on
	// the OS's file system.
	// 
	// If the resource already exists then this function should do nothing.
	// 
	// Some resources cannot be created, in this case an Error with the type ErrReadOnly or
	// ErrBadAction is returned (whichever one make more sense).
	...
	// ReadAll attempts to read an AXIS file into memory and returns the result and any error that
	// may have happened.
	...
	// WriteAll attempts to replace the contents of an AXIS file with the data given, if an error
	// happened it is returned.
...
type Collection interface {
	...
	// 
	// Mounting a new DataSource onto an existing location will generally replace the current
	// DataSource.
...
// Delete tries to delete an AXIS resource, this generally removes the backing file/directory from
// the OS's file system.
// 
...
// Create tries to create an AXIS resource, this generally creates the backing file/directory on the
// OS's file system.
// 
// If the resource already exists then this function should do nothing.
...
// ReadAll attempts to read an AXIS file into memory and returns the result and any error that may
// have happened.
...
// file/directory name, depending on context.
// 
// Mounting a new DataSource onto an existing location will generally replace the current DataSource.
