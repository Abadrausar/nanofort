...
type parserAlt struct {
	l *lexerAlt
}

// ParseC reads CLua source into an AST using the types in this package.
// This generates exactly the same kind of AST as created by the normal Parse function,
// but the excepted syntax is significantly different.
func ParseC(source string, line int) (block []Stmt, err error) {
	defer func(){
		if x := recover(); x != nil {
			switch e := x.(type) {
			case luautil.Error:
...
func (p *parserAlt) funcDeclStat(local bool) Stmt {
...
func (p *parserAlt) block() []Stmt {
	p.l.getCurrent(tknOBracket)
	rtn := []Stmt{}
	for !p.l.checkLook(tknCBracket, tknINVALID) {
...
func (p *parserAlt) statement() Stmt {
	...
	case tknIf:
		...
		rnode := node
		node.(*If).Then = p.block()
		loop:
		for {
			switch p.l.look.Type {
			case tknElse:
				if !p.l.checkLook(tknIf) {
					node.(*If).Else = p.block()
					break loop
				}
				line := p.l.current.Line
				pnode := node
	...
	case tknOBracket:
		line := p.l.look.Line
		return stmtLine(&DoBlock{Block: p.block()}, line)
	case tknFor:
	...
	case tknOParen:
		...
		ident := p.expression()
		s := Stmt(p.funcCall(ident).(*FuncCall))
