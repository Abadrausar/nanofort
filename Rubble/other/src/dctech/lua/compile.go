...
func (state *compState) localOK(n string) bool {
	bpc := int32(sliceutil.Top(&state.blocks).(*blockStuff).bpc)
	for _, l := range state.f.localVars {
		if l.sPC > l.ePC && l.ePC == bpc && l.name == n {
			return false
		}
	}
	return true
...
func compSourceC(source, name string, line int) (f *funcProto, err error) {
	// Quick-and-dirty error trapping.
	defer func(){
		if x := recover(); x != nil {
			switch e := x.(type) {
	...
	block, err := ast.ParseC(source, line)
	if err != nil {
		return nil, err
	}
	return compile(&ast.FuncDecl{Source: name, IsVariadic: true, Block: block}, nil), nil
