...
// LoadTextC loads a text chunk into memory and pushes the result onto the stack.
// If there is an error it is returned and nothing is pushed.
// Set env to 0 to use the default environment.
//
// This version is identical to LoadText except that it uses a significantly different syntax.
// The syntax supported is much closer to C (or Go), and as such should be easier for C (or Go)
// programmers to use.
func (l *State) LoadTextC(in io.Reader, name string, env int) error {
	source, err := ioutil.ReadAll(in)
	if err != nil {
		return err
	}
	proto, err := compSourceC(string(source), name, 1)
	if err != nil {
		return err
	}
...
// your path or is in the current working directory.
func (l *State) LoadTextExternal(in io.Reader, name string, env int) error {
	outFile := os.TempDir() + "/dctech.lua.bin"
